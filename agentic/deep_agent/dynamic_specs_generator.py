...
# Use Gemini 2.5 Pro for deep reasoning (better at discovery)
...
TASK: Analyze the product type and context to determine ALL relevant technical specification keys that should be captured for this product.
...
2. What technical parameters define this product's performance?
3. What safety/compliance specs are required?
4. What physical/mechanical specs matter?
5. What electrical/signal specs are relevant?
6. What environmental specs apply?
7. What material specs affect performance?
8. What maintenance/calibration specs are needed?

=== SPECIFICATION CATEGORIES TO CONSIDER ===

PERFORMANCE SPECS (if applicable):
- accuracy, repeatability, linearity, hysteresis
- measurement_range, span, rangeability, turndown_ratio
- resolution, sensitivity, drift, stability

ELECTRICAL SPECS (if applicable):
- output_signal, supply_voltage, power_consumption
- communication_protocol, loop_resistance
- isolation_voltage, grounding_requirements

PHYSICAL SPECS (if applicable):
- process_connection, mounting_type, dimensions
- weight, insertion_length, bore_size

MATERIAL SPECS (if applicable):
- material_wetted, material_housing, material_seal
- diaphragm_material, fill_fluid

ENVIRONMENTAL SPECS (if applicable):
- temperature_range, ambient_temperature, process_temperature
- humidity_range, protection_rating, vibration_resistance

SAFETY/COMPLIANCE SPECS (if applicable):
- hazardous_area_approval, sil_rating, certifications
- pressure_rating, burst_pressure

MAINTENANCE SPECS (if applicable):
- calibration_interval, service_life, warranty_period
- mtbf, maintenance_requirements

=== CRITICAL RULES ===

1. ONLY include specs that are RELEVANT to this specific product type
2. A Junction Box does NOT need "accuracy" or "measurement_range"
3. A Power Supply does NOT need "material_wetted" or "process_connection"
4. A Thermowell DOES need "bore_size", "insertion_length", "pressure_rating"
5. Be comprehensive but NOT generic - think about what an engineer would specify
...
TASK: Generate technical specification VALUES for the discovered keys.
...
MANDATORY (must provide values):
{mandatory_keys}

OPTIONAL (provide if applicable):
{optional_keys}

SAFETY CRITICAL (must provide if applicable):
{safety_keys}
...
CORRECT: "±0.1%", "IP67", "4-20mA HART", "-40 to +85°C", "SIL 2", "316L SS"
WRONG: "typically ±0.1%", "IP67 for outdoor use", "depends on application"

FORBIDDEN words in values: "typically", "usually", "approximately", "depends on", "varies"
...
{{
    ...
    "generation_notes": "Brief note on assumptions made"
}}

RULES:
1. Generate values ONLY for the keys provided above
2. Use "N/A" only if truly not applicable to this product
3. Do NOT add keys that weren't requested
4. Be specific to the product type - don't use generic values
"""
